				...
				<div id="keyword" class="card-tools" style="width: 550px;">
					<div class="input-group row">
						...
						<select class="form-control col-md-3" name="perPageNum"
							...
							<option value="2" ${pageMaker.perPageNum eq 2 ? 'selected':'' }>2개씩
								정렬</option>
							<option value="3" ${pageMaker.perPageNum eq 3 ? 'selected':'' }>3개씩
								정렬</option>
						</select>
						...
						<table class="table table-bordered">
							...
							<c:if test="${not empty memberList }">
								<c:forEach var="member" items="${memberList }">
									<tr
										onclick="OpenWindow('detail?id=${member.id}', '상세보기', 700, 800);"
										style="cursor: pointer;">
										<td style="margin: 0; padding: 0; padding-top: 5px;"><span
											class="manPicture" data-id="${member.id}"
											style="display: block; width: 40px; height: 40px; margin: 0"></span>
										</td>
										<td>${member.id }</td>
										<td>${member.pwd }</td>
										<td>${member.name }</td>
